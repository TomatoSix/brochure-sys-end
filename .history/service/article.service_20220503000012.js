...
class articleService {
  ...
  async reviseChapterContent(data) {
    ...
    const statement = `update chapter set title="设计模式2", sort=2 where chapterId=1;
    update article set content='内容修改2' where articleId = (
      select articleId from chapter c where c.chapterId = 1)`;
    const result = await connection.execute(statement, [
      content,
      title,
      sort,
