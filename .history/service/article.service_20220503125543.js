...
class articleService {
  ...
  async reviseChapterContent(data) {
    ...
    console.log(data, "data");
    const articleId = await findArticleId(chapterId);
    const statement = `update chapter set title="再来一次", sort=0 where chapterId=1;
    update article set content='继续修改，不管了' where articleId = 1
    `;
    const result = await connection.execute(statement, [
      title,
      sort,
