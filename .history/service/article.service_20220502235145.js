...
class articleService {
  ...
  async getChapterAllByBrochureId(id) {
    const statement = `SELECT c.chapter, c.user_id, c.brochureId, c.articleId, c.title, c.title, a.content, from chapter c left join article a
    on c.articleId = a.articleId
    WHERE c.brochureId = 2 order by c.sort asc;`;
    const result = await connection.execute(statement, [id]);
    return result;
  }
  // 添加章节
  ...
  async reviseChapterContent(data) {
    ...
    const statement = `update chapter set content=?, title=?, sort=? where chapterId=?;`;
    const result = await connection.execute(statement, [
      content,
      title,
      sort,
