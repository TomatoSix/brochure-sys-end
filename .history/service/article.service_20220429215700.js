...
class articleService {
  ...
  async getArticleDataAll() {
    const statement = `
    SELECT a.articleId articleId, a.digest digest, u.name authorName, a.title title, a.content content, a.likes likes, a.comments comments FROM  users u RIGHT JOIN article a on u.id = a.user_id；`;
    const result = await connection.execute(statement);
    return result;
  }
}
